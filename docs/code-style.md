...
- **Go 1.26+**: указатель на значение/литерал/**выражение** получай через `new(expr)`, а не заводи временную переменную только ради взятия её адреса (`tmp := expr; &tmp`) и не через `lo.ToPtr(...)`. Примеры: `obj.Active = new(true)`, `pars.Page = new(int32(1))`, `res.Status = new(int(sv.ToInteger()))`. Правило действует **во всём проекте**, а не только в CRUD. Исключение — когда выражение возвращает ещё и `error` (`v, err := f()`): переменную приходится завести ради обработки ошибки, тогда `&v` допустим.
...
- Для ошибок — `errors.Is` / `errors.AsType`, без прямого `==` и type assertion `err.(*T)`: так корректно обрабатываются обёрнутые ошибки.
- Worker pool / параллельную обработку делай через **`errgroup`** (`golang.org/x/sync/errgroup`), а не ручным `sync.WaitGroup` + каналы (пример — `internal/usecase/app/swagger_discovery.go`).
...
- **REST-пути всегда в единственном числе**, без plural: `/app`, `/app/{id}`, `/endpoint`, `/usr`
  (не `/apps`, `/endpoints`, `/users`).
- **Для update — HTTP-метод `PUT`** (не `PATCH`): `put: "/<entity>/{id}"`.
- **Пагинация: `page` начинается с `0`** (нумерация с нуля). `ListParams.Page` уходит в mobone
  как 0-based offset; не прибавляй/вычитай `1` ни на бэке, ни на фронте.

## Миграции

- Файлы в `migrations/` в формате `<timestamp>_<name>.up.sql` / `.down.sql` (golang-migrate).
- **Имена таблиц всегда в единственном числе**: `root`, `app`, `endpoint`, `snapshot`, `usr`
  (не plural). То же значение — в `TableName` репозитория.
- В `down`-миграциях во всех командах `DROP` обязательно указывай `CASCADE`
  (`drop table if exists <t> cascade;`, `DROP COLUMN <c> CASCADE`, `DROP FUNCTION <f>(...) CASCADE`).
- В `down` объекты удаляй в порядке, **обратном** `up` (с учётом внешних ключей).
